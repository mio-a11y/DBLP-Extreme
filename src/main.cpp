...
void print_menu() {
    ...
    std::cout << " [5] F3. 作者统计 (Top 100)\n";
    std::cout << " [6] F4. 年度热词分析\n";
    std::cout << " [7] F6. 聚团分析\n";
    std::cout << " [8] F7. 可视化数据导出 -> /frontend/\n";
...
int main(int argc, char* argv[]) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (arg.rfind("--f6", 0) == 0) {
            f6_mode = true;
            const std::string_view rest(arg.data() + 4, arg.size() - 4);
            if (rest.size() >= 2 && rest[0] == '=') {
                const std::string_view val(rest.data() + 1, rest.size() - 1);
                try {
                    const unsigned long parsed = std::stoul(std::string(val));
                    f6_max_order = static_cast<std::uint32_t>(std::clamp<unsigned long>(parsed, 2UL, 128UL));
                } catch (const std::exception&) {
                    f6_max_order = 8;
                }
            } else if (!rest.empty()) {
                try {
                    const unsigned long parsed = std::stoul(std::string(rest));
                    f6_max_order = static_cast<std::uint32_t>(std::clamp<unsigned long>(parsed, 2UL, 128UL));
                } catch (const std::exception&) {
                    f6_max_order = 8;
                }
    ...
    if (f6_mode) {
        const auto t0 = std::chrono::steady_clock::now();
        engine.execute_f6_global_ranking(f6_max_order);
        const auto t1 = std::chrono::steady_clock::now();
        const auto ms = std::chrono::duration_cast<std::chrono::milliseconds>(t1 - t0).count();
        std::cout << "[计时] F6 聚团统计耗时: " << ms << " ms\n";
        if (!f7_mode) return 0;
    }
    ...
    if (f7_mode) {
        ...
        const auto t1 = std::chrono::steady_clock::now();
        const auto ms = std::chrono::duration_cast<std::chrono::milliseconds>(t1 - t0).count();
        std::cout << "[计时] F7 导出耗时: " << ms << " ms\n";
        return 0;
    }

    if (serve_mode) {
        std::cout << "[SERVE] Ready\n" << std::flush;
        std::string line;
        while (std::getline(std::cin, line)) {
            const std::string_view cmd = [](std::string_view s) {
                std::size_t b = 0;
            ...
            if (cmd.starts_with("AUTHOR ")) {
                engine.serve_search_author(cmd.substr(7), std::cout);
            } else if (cmd.starts_with("TITLE ")) {
                engine.serve_search_title(cmd.substr(6), std::cout);
            } else if (cmd.starts_with("BM25 ")) {
                engine.serve_search_bm25(cmd.substr(5), std::cout);
            } else if (cmd.starts_with("CLIQUE ")) {
                std::uint32_t order = 5;
                try {
                    const unsigned long parsed = std::stoul(std::string(cmd.substr(7)));
                    order = parsed < 2 ? 2u : (parsed > 128 ? 128u : static_cast<std::uint32_t>(parsed));
                } catch (...) {}
